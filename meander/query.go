...
// APIKey is place api authorization key
var APIKey string

// Place is place information
type Place struct {
...
func (q *Query) Run() []interface{} {
	...
	for i, r := range q.Journey {
		...
		go func(types string, i int) {
			...
			if err != nil {
				log.Println("Failed to search location", err)
				return
			}
			if len(response.Results) == 0 {
				log.Println("Failed to find location", err)
				return
			}
